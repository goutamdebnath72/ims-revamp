...
// *** FIX: NEW HELPER FUNCTION TO GET DATE FROM THE RELIABLE 'reportedOn' FIELD ***
const getDateFromReportedOn = (reportedOnString) => {
  if (typeof reportedOnString !== 'string') return null;
  try {
    // This format matches "20 Jul 2025 00:48"
    const date = DateTime.fromFormat(reportedOnString, 'dd MMM yyyy HH:mm', { zone: 'Asia/Kolkata' });
...
export const filterIncidents = (incidents, criteria, user) => {
  return incidents.filter(incident => {
    // *** FIX: USE THE NEW HELPER FUNCTION FOR ACCURATE DATE/TIME ***
    const reportedDate = getDateFromReportedOn(incident.reportedOn);
    if (!reportedDate) return false;

    // 1. Date Range Check using Luxon (Now works correctly)
    ...
    // 2. Shift Check using Luxon (Now works correctly)
