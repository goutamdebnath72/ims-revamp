...
export const filterIncidents = (incidents, criteria, user) => {
  return incidents.filter((incident) => {
    ...
    // Add a check to see if we should ignore date filtering
    const isAllTime = criteria.dateRange.start.toISODate() === "2020-01-01";
    ...
    if (user) {
      ...
      if (user.role === "user" && incident.requestor.name !== user.name)
        return false;
