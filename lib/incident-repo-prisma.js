...
export async function getAllIncidents(filters = {}, user) {
  ...
  // ===== THIS IS THE FIX for the Telecom Dashboard =====
  if (user?.role === "standard") {
    where.requestor = { ticketNo: user.ticketNo };
  } else if (user?.id === "telecom") {
    // We now check for the user's ID, not their role
  ...
  if (status && status !== "Any") {
    if (status === "open") {
      where.status = {
        in: isVendor
          ? ["Processed", "Pending Telecom Action"]
          : ["New", "Processed"],
      };
    } else {
      where.status = status;
  ...
  // ... The rest of the function remains the same, no changes needed below this line in this function
  ...
  console.log("[API DEBUG] Running prisma.findMany with WHERE clause:", JSON.stringify(where, null, 2));
  // -----------------------------
