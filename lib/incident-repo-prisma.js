...
import { INCIDENT_STATUS, USER_ROLES, TEAMS, INCIDENT_TYPES } from "@/lib/constants";

// This function fetches all incidents and includes all related data
export async function getAllIncidents(filters = {}, user) {
  console.log("DEBUG: 10. [Dashboard Data Fetch]", { user });
  const {
    page = 1,
    limit = "20",
    status,
  ...
  // =======================================================
  // ===== WINNING LOGIC w/ CASE-INSENSITIVE FIX ===========
  // =======================================================
  if (user?.role === USER_ROLES.STANDARD) {
    where.requestor = { ticketNo: user.ticketNo };
  } else if (user?.role === USER_ROLES.TELECOM_USER) {
    // Stricter logic is now robust against case sensitivity
    where.status = {
      equals: INCIDENT_STATUS.PENDING_TELECOM_ACTION,
      mode: 'insensitive', // This ignores case for the status field
    };
    where.incidentType = {
      name: {
        equals: INCIDENT_TYPES.NETWORK,
        mode: 'insensitive', // This ignores case for the incident type name
      },
    };
  } else {
    // This is the general logic for admins
    ...
    if (status === "open") {
      where.status = {
        in: isVendor
          ...
          : [INCIDENT_STATUS.NEW, INCIDENT_STATUS.PROCESSED],
      };
    } else {
      where.status = status;
    }
  } else {
    // This is a special case to prevent vendors from seeing 'New' incidents
    // unless they are filtered by the 'open' status above.
    if (isVendor) {
      // If a telecom user is also considered a vendor, this `where.status` would overwrite
      // the specific status filter applied in the TELECOM_USER block.
      // We need to ensure the telecom user's specific filter is not overwritten.
      if (user?.role !== USER_ROLES.TELECOM_USER) {
         where.status = { not: INCIDENT_STATUS.NEW };
      }
    }
  }
  ...
  // This block should not run if the type is already set by the TELECOM_USER logic
  ...
  console.log("[API DEBUG] Running prisma.findMany with WINNING WHERE clause:", JSON.stringify(where, null, 2));
...
// In lib/incident-repo-prisma.js, replace the entire updateIncident function

export async function updateIncident(incidentId, updatedData, user) {
    console.log("DEBUG: 5. [Repo updateIncident Called]", { incidentId, updatedData });
  ...
  // ===== THIS IS THE FIX for saving the referral data =====
  const updatedIncident = await prisma.$transaction(async (tx) => {
    ...
    // This handles re-opening a resolved incident
    if (
    ...
    // Handles priority changes by admin
    ...
    // Set correct action text for resolution/closing
    ...
    // Step 1: Update the main incident fields if there's anything to update
    ...
    // Step 2: Always create the related audit trail entry
    ...
    // Step 3: Fetch and return the fully updated incident
