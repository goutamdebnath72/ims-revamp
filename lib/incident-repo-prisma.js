...
export async function getAllIncidents(filters = {}, user) {
  ...
  if (status && status !== "Any") {
    if (status === "open") {
      ...
      // FIX 1: Made the general status filter case-insensitive for robustness
  ...
  if (priority && priority !== "Any") {
    // FIX 2: Made the priority filter case-insensitive
    where.priority = { equals: priority, mode: "insensitive" };
  }

  if (incidentType && incidentType !== "Any" && !where.incidentType) {
    // FIX 3: Made the main incidentType filter case-insensitive
    where.incidentType = {
  ...
  // The rest of the function for pagination remains the same...
...
export async function updateIncident(incidentId, updatedData, user) {
  console.log("DEBUG: 5. [Repo updateIncident Called]", {
    incidentId,
    updatedData,
  });
  ...
  const updatedIncident = await prisma.$transaction(async (tx) => {
    // --- START: NEW LOGIC FOR DELIBERATE UNLOCK ---
    if (updatedData.action === "UNLOCK_TYPE") {
      // Step 1: Unlock the incident type
      await tx.incident.update({
        where: { id: incidentId },
        data: { isTypeLocked: false },
      });

      // Step 2: Create a specific audit trail entry for this action
      ...
      // Step 3: Fetch and return the newly unlocked incident
    ...
    // --- END: NEW LOGIC FOR DELIBERATE UNLOCK ---
