...
export default function BiometricVendorDashboard() {
  ...
  const biometricIncidents = React.useMemo(() => {
    return incidents.filter(
      (incident) =>
        incident.incidentType === "BIOMETRIC" && incident.status !== "New"
    );
  }, [incidents]);

  const filteredIncidents = React.useMemo(() => {
    const criteria = { dateRange, shift };
    return filterIncidents(biometricIncidents, criteria, user);
  }, [biometricIncidents, dateRange, shift, user])
  ...
  const constructCardUrl = (status) => {
    ...
    params.append("incidentType", "BIOMETRIC"); // Always filter for network
    if (shift !== "All") {
